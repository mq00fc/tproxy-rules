#!/bin/sh
# Private
nft add element inet proxy private_ips  {  }
nft add element inet proxy private_ips6 {  }
# Telegram
nft add element inet proxy ips { 149.154.160.0/20,185.76.151.0/24,91.105.192.0/23,91.108.12.0/22,91.108.16.0/22,91.108.20.0/22,91.108.4.0/22,91.108.56.0/22,91.108.8.0/22 }
nft add element inet proxy ips6 { 2001:67c:4e8::/48,2001:b28:f23c::/48,2001:b28:f23d::/48,2001:b28:f23f::/48,2a0a:f280::/32 }
# Cloudflare
nft add element inet proxy ips {  }
nft add element inet proxy ips6 {  }
# GitHub
nft add element inet proxy ips {  }
nft add element inet proxy ips6 {  }
# Google
nft add element inet proxy ips {  }
nft add element inet proxy ips6 {  }
# Facebook
nft add element inet proxy ips {  }
nft add element inet proxy ips6 {  }
# CloudFront
nft add element inet proxy ips {  }
nft add element inet proxy ips6 {  }
# Netflix
nft add element inet proxy ips {  }
nft add element inet proxy ips6 {  }
